...
""",
    ...
    ollama_parser = subparsers.add_parser(
        "ollama",
        help="Inspect local Ollama runtime state",
    )
    ollama_sub = ollama_parser.add_subparsers(dest="ollama_command")
    ollama_residency = ollama_sub.add_parser(
        "residency",
        help="Report loaded Ollama models and expected memory-stack residency",
    )
    ollama_residency.add_argument("--json", action="store_true", help="Output JSON")
    ollama_residency.add_argument("--fix", action="store_true", help="Unload transient embedding model if resident")

    def cmd_ollama(args):
        sub = getattr(args, "ollama_command", None)
        if sub == "residency":
            from hermes_cli.ollama_residency_cmd import ollama_residency_command

            ollama_residency_command(args)
            return
        ollama_parser.print_help()
