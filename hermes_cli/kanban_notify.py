...
def maybe_auto_subscribe_create(conn: Any, task_id: str) -> bool:
    ...
    try:
        ...
        for key, env_key in (
            ...
            ("parent_chat_id", "HERMES_SESSION_PARENT_CHAT_ID"),
        ):
            value = get_session_env(env_key, "")
            if value:
        ...
        # Creator-lineage inheritance may already have installed the same
        # destination with an explicit delivery policy. Do not upgrade or
        # rewrite that durable policy from ambient session context.
        if any(
            sub["platform"] == str(platform)
            and sub["chat_id"] == str(chat_id)
            and (sub["thread_id"] or "") == (thread_id or "")
